...
export function buildSelectOptionsWithDisabled(
  input: any[],
  ids: number[] = []
): SelectsWDisable[] | any[] {
  ...
  if (input.length) {
    ...
    if (enable.length)
      output.push({
        array: enable,
        disabled: false,
        name: 'general.select_enabled',
      });

    let disabled = input.filter(
      (x) => x.indActivo === false && ids.includes(x.id)
    );
...
export const customPositiveCurrencyMaskConfig = {
  prefix: '$',
  thousands: '.',
  decimal: ',',
  allowNegative: false,
};
export const customNegativeCurrencyMaskConfig = {
  prefix: '-$',
  thousands: '.',
  decimal: ',',
  allowNegative: false,
  ...
  if (user) {
    user.permissions.forEach((permission) => {
      if (permission.module == Modules.GESTAO) {
        ...
        if (
          permission.idFuncionalidade ==
          Funcionalidades.ControlodeAcessodeUtilizadores
        ) {
          response
            .find((x) => x.name == gestaoTitle)
            ?.children?.push({
...
export function showExpiredError(
...
export function populateChartColors(
  chartColors: any[],
  ...
  for (let i = 0; i < numberColors; i++) {
    ...
    let colorString =
      'rgba(' + color.values.rgb + ',' + color.values.alpha + ')';
